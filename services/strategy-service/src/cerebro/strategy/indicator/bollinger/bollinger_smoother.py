...
class BollingerSmoother(bt.Indicator):
    ...
    def __init__(self):
        ...
        bb = bt.indicators.BollingerBands(
            self.data, period=self.p.period, devfactor=self.p.stdev
        )
        self.l.mid = NadarayaWatsonSmoother(
            bb.l.mid, window=self.p.period, bandwidth=self.p.smooth_factor
        )

        self.l.bolu, self.l.bold = self.smooth_bollinger(
            self.data, self.p.period, self.p.stdev
        )
