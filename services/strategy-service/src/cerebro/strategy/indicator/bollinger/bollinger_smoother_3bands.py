...
class BollingerSmoother3bands(bt.Indicator):
    ...
    params = (
        (
            "repaint",
            False,
        ),  # "Repaint Smoothing", tooltip = "This setting allows for repainting of the estimation"
        # default smoothing nadaraya smoothing length
        ("smooth_dist", 500),
        (
            "smooth_factor",
            6,
        ),  # "Smoothing Factor", tooltip = "Smoothing factor for the Nadaraya-Watson estimator"
        ("sens", 4),
    ...
    plotlines = dict(
        ...
        bold_3=dict(
            markersize=0.0, color="#FFFFFF", _fill_gt=("bold_2b", ("#4CAF50", 0.20))
        ),
    )
    ...
    def smooth_bollinger(self, data, period, dev_factor):
        ...
        top = NadarayaWatsonSmoother(
            bb.l.top, window=period, bandwidth=self.p.smooth_factor
        )
        bot = NadarayaWatsonSmoother(
            bb.l.bot, window=period, bandwidth=self.p.smooth_factor
        )
    ...
    def __init__(self):
        ...
        bb = bt.indicators.BollingerBands(
            self.data, period=self.p.short_period, devfactor=self.p.short_stdev
        )
        self.l.mid = NadarayaWatsonSmoother(
            bb.l.mid, window=self.p.short_period, bandwidth=self.p.smooth_factor
        )

        self.l.bolu_1, self.l.bold_1 = self.smooth_bollinger(
            self.data, self.p.short_period, self.p.short_stdev
        )
        self.l.bolu_2, self.l.bold_2 = self.smooth_bollinger(
            self.data, self.p.med_period, self.p.short_stdev
        )
        self.l.bolu_2b, self.l.bold_2b = self.smooth_bollinger(
            self.data, self.p.med_period, self.p.short_stdev
        )
        self.l.bolu_3, self.l.bold_3 = self.smooth_bollinger(
            self.data, self.p.long_period, self.p.med_stdev
        )
