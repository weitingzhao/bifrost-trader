...
class Funding(models.Model):
    ...
    funding_type = models.CharField(
        max_length=20,
        choices=FundingTypeChoices.choices,
        default=FundingTypeChoices.NONE,
    )
...
class Holding(models.Model):
    ...
    symbol = models.ForeignKey(
        MarketSymbol, to_field="symbol", on_delete=models.DO_NOTHING
    )
...
class Trade(models.Model):
    ...
    trade_phase = models.CharField(
        max_length=20, choices=TradePhaseChoices.choices, default=TradePhaseChoices.NONE
    )
    trade_phase_rating = models.CharField(
        max_length=20,
        choices=TradePhaseRatingChoices.choices,
        default=TradePhaseRatingChoices.NONE,
    )
    trade_source = models.CharField(
        max_length=20,
        choices=TradeSourceChoices.choices,
        default=TradeSourceChoices.PLATFORM_A,
    )
...
class CashBalance(models.Model):
    ...
    portfolio = models.ForeignKey(
        Portfolio, on_delete=models.DO_NOTHING, null=True, blank=True
    )
    money_market = models.DecimalField(
        max_digits=15, decimal_places=2, default=0.00, null=True
    )
...
class Transaction(models.Model):
    ...
    transaction_type = models.CharField(
        ...
        default=TransactionTypeChoices.NONE,
    )
    ...
    trade = models.ForeignKey(
        Trade, on_delete=models.DO_NOTHING, null=True, blank=True
    )  # Add trade_id field
...
class HoldingOrder(models.Model):
    ...
    price_stop = models.DecimalField(
        max_digits=10, decimal_places=2, null=True, blank=True
    )
    ...
    action = models.CharField(
        max_length=20, choices=ActionChoices.choices, default=ActionChoices.NONE
    )
    timing = models.CharField(
        max_length=20, choices=TimingChoices.choices, default=TimingChoices.NONE
    )
...
class Order(HoldingOrder):
    ...
    order_style = models.CharField(
        max_length=20, choices=OrderStyleChoices.choices, default=ActionChoices.NONE
    )

    wishlist = models.ForeignKey(
        Wishlist, on_delete=models.DO_NOTHING, null=True, blank=True
    )
