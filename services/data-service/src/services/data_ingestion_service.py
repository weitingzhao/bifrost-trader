...
class DataIngestionService:
    ...
    async def ingest_symbol_info(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            with self.SessionLocal() as session:
                ...
                existing_symbol = (
                    session.query(MarketSymbolModel)
                    .filter(MarketSymbolModel.symbol == symbol)
                    .first()
                )

                if existing_symbol:
                    # Update existing symbol
                    existing_symbol.name = company_info.get(
                        "name", existing_symbol.name
                    )
                    existing_symbol.market = company_info.get(
                        "exchange", existing_symbol.market
                    )
                    existing_symbol.asset_type = "Stock"  # Default to Stock
                    existing_symbol.sector = company_info.get(
                        "sector", existing_symbol.sector
                    )
                    existing_symbol.industry = company_info.get(
                        "industry", existing_symbol.industry
                    )
                    existing_symbol.country = company_info.get(
                        "country", existing_symbol.country
                    )
                    existing_symbol.currency = company_info.get(
                        "currency", existing_symbol.currency
                    )
                    existing_symbol.description = company_info.get(
                        "description", existing_symbol.description
                    )
    ...
    async def ingest_historical_data(
        ...
        try:
            ...
            historical_data = await self.yahoo_service.get_historical_data(
                symbol, period
            )
            ...
            with self.SessionLocal() as session:
                for i in range(0, len(data_points), batch_size):
                    ...
                    self.logger.info(
                        f"Inserted batch {i//batch_size + 1} for {symbol}: {len(batch)} records"
                    )
            ...
            with self.SessionLocal() as session:
                ...
                existing_data = (
                    ...
                    .filter(
                        ...
                        MarketDataModel.timestamp
                        >= latest_timestamp - timedelta(minutes=5),
                    )
                    .first()
                )
    ...
    async def batch_ingest_symbols(self, symbols: List[str]) -> Dict[str, Any]:
        ...
        try:
            results = {
                ...
                "results": {},
                "errors": {},
                "summary": {"total": len(symbols), "success": 0, "errors": 0},
            }
    ...
    async def update_symbol_data(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            results = {"symbol": symbol, "tasks": {}, "status": "success"}
        ...
        try:
            with self.SessionLocal() as session:
                ...
                return {
                    ...
                    "latest_data_timestamp": latest_data.timestamp.isoformat()
                    if latest_data
                    else None,
